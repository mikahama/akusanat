...
class WikiTool:
    ...
    def __init__(self):
        self.wiki_url = "https://akusanat.com/"
        self.username = "Synkkis"
        self.password = "SyncBot12"
        self.login_cookies = {}
        self.token = ""
        self.cookies = {}
    ...
    def delete_page(self, name):
        postParameters = {"action": "delete", "title": name, "token": self.token, "format": "json"}
        success, results, c = self.post(self.wiki_url + "api.php", postParameters)
        return success, results
...
wt.get_token()
for lang in ["sms", "kpv"]:
	wrongs = json_load(lang + "_wrong.json")
	for w in wrongs:
		print(w, wt.delete_page(w)[0])